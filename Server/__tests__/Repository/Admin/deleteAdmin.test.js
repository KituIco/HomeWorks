...
describe('AdminRepository', () => {
    ...
    describe('deleteAdmin', () => {
        ...
        it('should not delete any admin if invalid admin ID is provided', async () => {
            ...
            const initialAdminCount = await db.query(
            ...
            const finalAdminCount = await db.query(
                'SELECT COUNT(*) as count FROM Admin'
            );

            // Assert that the count remains unchanged
            expect(finalAdminCount[0][0].count).toBeLessThanOrEqual(
                initialAdminCount[0][0].count
            );
        });
