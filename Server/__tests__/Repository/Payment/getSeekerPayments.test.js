...
describe('Payment Repository', () => {
    ...
    describe('getSeekerPayments', () => {
        it('should return an array of payments for a given seeker', async () => {
            const payments = await paymentRepo.getSeekerPayments(seekerID);
        ...
        it('should return an empty array if no payments are found', async () => {
            const payments = await paymentRepo.getSeekerPayments('invalid');

            expect(payments).toEqual([]);
        });

        it('should throw an error if the seekerID is too long', async () => {
            await expect(
                paymentRepo.getSeekerPayments('a'.repeat(50))
            ).rejects.toThrow();
        });
    });
});
