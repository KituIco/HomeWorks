...
describe('Payment Repository', () => {
    ...
    describe('getPaymentByMethod', () => {
        it('should return payment by method given valid method', async () => {
            const payment = await paymentRepo.getPaymentByMethod(1);

            payment.forEach((payment) => {
                expect(payment.paymentMethod).toBe(1);
            });
        });

        it('should return null given invalid method', async () => {
            await expect(
                paymentRepo.getPaymentByMethod('test')
            ).rejects.toThrow();
        });

        it('should return null given null method', async () => {
            let payment = await paymentRepo.getPaymentByMethod(null);
