...
describe('PayoutRepository', () => {
    ...
    describe('getSeekerPayouts', () => {
        it('should return an array of payouts if seekerID is valid', async () => {
            // Call the getSeekerPayouts function
            const payouts = await payoutRepo.getSeekerPayouts(seekerID);
        ...
        it('should return an empty array if seekerID is invalid', async () => {
            // Call the getSeekerPayouts function
            const payouts = await payoutRepo.getSeekerPayouts('invalid');
        ...
        it('should return an error if seekerID length is greater than 14', async () => {
            // Call the getSeekerPayouts function
            await expect(
                payoutRepo.getSeekerPayouts('invalidinvalidinvalid')
            ).rejects.toThrow();
        });
    });
});
